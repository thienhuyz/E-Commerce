...
import { apiGetBrands } from "../../apis/brand"; // API lấy brand
...
const ProductsPage = () => {
  ...
  const [brands, setBrands] = useState([]); // thêm brands
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setCategories(catRes.prodCategories || []);

        const brandRes = await apiGetBrands();
    ...
    <div className="p-4">
      ...
      >
        <ProductForm
          ...
          brands={brands} // truyền brands cho form
          onSuccess={() => {
