...
const MemberSidebar = () => {
    ...
    const fullName = `${current?.firstname || ''} ${current?.lastname || ''}`.trim() || 'Thành viên'

    return (
        <aside className='bg-white h-full w-[260px] flex-none border-r border-slate-100'>
            {/* Header user */}
            <div className='flex flex-col items-center justify-center gap-3 py-6 px-4'>
                ...
                <div className='text-center'>
                    <p className='text-base font-semibold text-slate-800 line-clamp-1'>{fullName}</p>
                    {current?.email && <p className='text-xs text-slate-500 line-clamp-1'>{current.email}</p>}
                </div>
            </div>
                            ...
                            <div className='rounded-xl border border-slate-100 overflow-hidden'>
                                ...
                                >
                                    <div className='flex items-center gap-2'>
                                        <span className='text-lg'>{el.icon}</span>
                                        <span className='text-sm font-semibold text-slate-800'>{el.text}</span>
                                    </div>

                                    {/* caret quay xuống khi mở */}
                                    <AiOutlineCaretDown
                                        className={clsx(
                                ...
                                {isOpen(el.id) && (
                                    <div
                                        id={`submenu-${el.id}`}
                                        className='flex flex-col py-2 bg-white'
                                    >
                                        {el.submenu.map((item) => (
                                            <NavLink
