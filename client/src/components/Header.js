...
const Header = () => {
    ...
    useEffect(() => {
        if (mes)
            Swal.fire('Rất tiếc!', mes, 'info').then(() => {
                dispatch(clearMessage())
                navigate(`/${path.LOGIN}`)
            })
    }, [mes])

    // click ngoài dropdown sẽ ẩn đi
    useEffect(() => {
    ...
    const onSubmitSearch = () => {
        ...
        if (keyword) navigate(`/search?title=${encodeURIComponent(keyword)}`)
    }
            ...
            <div className="w-main flex justify-between h-[72px] py-[35px] items-center">
                ...
                <Link to={`/${path.HOME}`}>
                    <img
                        src={logo}
                        alt="Logo"
                        className="w-[220px] object-contain mb-2"
                    />
                </Link>
                ...
                <div className="flex text-[13px]">
                    {isLoggedIn && current ? (
                        ...
                        >
                            ...
                            <span className="text-lg text-white">{`${current?.cart?.length || 0
                                } sản phẩm`}</span>
                        </div>
                    ) : (
                    ...
                    >
                        {isLoggedIn && current ? (
                            <div
                                ...
                                id="profile"
                            >
                                <span className="flex text-lg">{`${current?.firstname} ${current?.lastname}`}
                                    <FaUserCircle className="ml-4" color="white" size={30} />
                                </span>

                                {isShowOption && (
                                    <div
                                        onClick={(e) => e.stopPropagation()}
                                        className="absolute flex-col flex m-auto top-[30px] left-[140px] bg-red-500 border min-w-[140px] py-2 z-50 rounded-xl"
                                    >
                                        <Link
                            ...
                            <Link
                                className="cursor-pointer flex mx-4 text-lg items-center"
                                to={`/${path.LOGIN}`}
                            >
                                Đăng nhập{' '}
                                <FaUserCircle className="ml-4" color="white" size={30} />
